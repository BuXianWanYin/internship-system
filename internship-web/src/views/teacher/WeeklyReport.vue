  ...
  <PageLayout title="周报批阅">
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item label="周报信息">
          <div style="padding: 10px; background: #f5f7fa; border-radius: 4px">
            <div><strong>学生：</strong>{{ currentReport.studentName }}（{{ currentReport.studentNo }}）</div>
            <div style="margin-top: 5px"><strong>周次：</strong>第{{ currentReport.weekNumber }}周</div>
            <div style="margin-top: 5px"><strong>企业：</strong>{{ currentReport.enterpriseName || '-' }}</div>
          </div>
        </el-form-item>
...
const handleReviewFromDetail = () => {
  currentReport.value = detailData.value
  reviewForm.reviewScore = null
  reviewForm.reviewComment = ''
...
const handleSubmitReview = async () => {
  ...
  await reviewFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const res = await weeklyReportApi.reviewReport(
        ...
        if (res.code === 200) {
          ...
          loadData()
          // 如果是从详情对话框打开的，重新加载详情
          if (detailDialogVisible.value === false && currentReport.value.reportId === detailData.value.reportId) {
            handleView(currentReport.value)
          }
        }
      } catch (error) {
