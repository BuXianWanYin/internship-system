...
import SchoolEvaluationForm from '@/components/business/SchoolEvaluationForm.vue'
...
const evaluationDialogTitle = ref('学校评价')
...
const handleEvaluate = async (row) => {
  ...
  evaluationDialogTitle.value = `学校评价 - ${row.studentName}（学号：${row.studentNo}）`
  
  // 加载已有评价（草稿）
  const res = await schoolEvaluationApi.getEvaluationByApplyId(row.applyId)
...
const handleSaveEvaluation = async (data) => {
  try {
    const res = await schoolEvaluationApi.saveOrUpdateEvaluation(data)
    if (res.code === 200) {
...
const handleSubmitEvaluation = async (evaluationId) => {
  try {
    const res = await schoolEvaluationApi.submitEvaluation(evaluationId)
    if (res.code === 200) {
