...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public InternshipDurationStatisticsDTO getInternshipDurationStatistics(StatisticsQueryDTO queryDTO) {
        ...
        Map<String, List<Long>> monthDurations = applies.stream()
                ...
                .collect(Collectors.groupingBy(
                    a -> a.getInternshipStartDate().format(DateTimeFormatter.ofPattern("yyyy-MM")),
                    Collectors.mapping(
    ...
    public MajorStatisticsDTO getMajorStatistics(StatisticsQueryDTO queryDTO) {
        ...
        List<Student> students = studentMapper.selectList(
                new LambdaQueryWrapper<Student>()
                        ...
                        .select(Student::getStudentId, Student::getMajorId, Student::getUserId)
        );
    ...
    public ClassStatisticsDTO getClassStatistics(StatisticsQueryDTO queryDTO) {
        ...
        List<Student> students = studentMapper.selectList(
                new LambdaQueryWrapper<Student>()
                        ...
                        .select(Student::getStudentId, Student::getClassId, Student::getUserId)
        );
        // 通过关联user_info表过滤已删除的学生
        if (students != null && !students.isEmpty()) {
            List<Long> userIds = students.stream()
        ...
        // 按班级ID分组统计人数
        Map<Long, Long> classCountMap = null;
        if (students != null) {
            classCountMap = students.stream()
    ...
    public CollegeStatisticsDTO getCollegeStatistics(StatisticsQueryDTO queryDTO) {
        ...
        List<Student> students = studentMapper.selectList(
                new LambdaQueryWrapper<Student>()
                        ...
                        .select(Student::getStudentId, Student::getCollegeId, Student::getUserId)
        );
        // 通过关联user_info表过滤已删除的学生
        if (students != null && !students.isEmpty()) {
            List<Long> userIds = students.stream()
    ...
    public StudentScoreRankingDTO getStudentScoreRanking(StatisticsQueryDTO queryDTO) {
        ...
        if (queryDTO != null && queryDTO.getClassId() != null) {
            ...
            List<Student> students = studentMapper.selectList(
                    new LambdaQueryWrapper<Student>()
                            .eq(Student::getClassId, queryDTO.getClassId())
    ...
    private LambdaQueryWrapper<InternshipApply> buildApplyQueryWrapper(StatisticsQueryDTO queryDTO) {
        ...
        if (queryDTO != null) {
            ...
            if (queryDTO.getSchoolId() != null) {
                ...
                List<Student> students = studentMapper.selectList(
                        new LambdaQueryWrapper<Student>()
                                .eq(Student::getSchoolId, queryDTO.getSchoolId())
                                .select(Student::getStudentId, Student::getUserId)
                );
                // 通过关联user_info表过滤已删除的学生
                if (students != null && !students.isEmpty()) {
            ...
            if (queryDTO.getCollegeId() != null) {
                ...
                List<Student> students = studentMapper.selectList(
                        new LambdaQueryWrapper<Student>()
                                .eq(Student::getCollegeId, queryDTO.getCollegeId())
                                .select(Student::getStudentId, Student::getUserId)
                );
                // 通过关联user_info表过滤已删除的学生
                if (students != null && !students.isEmpty()) {
            ...
            if (queryDTO.getMajorId() != null) {
                ...
                List<Student> students = studentMapper.selectList(
                        new LambdaQueryWrapper<Student>()
                                .eq(Student::getMajorId, queryDTO.getMajorId())
                                .select(Student::getStudentId, Student::getUserId)
                );
                // 通过关联user_info表过滤已删除的学生
                if (students != null && !students.isEmpty()) {
    ...
    public SystemAdminDashboardDTO getSystemAdminDashboard() {
        ...
        for (School school : schools) {
            ...
            List<Student> students = studentMapper.selectList(
                    new LambdaQueryWrapper<Student>()
                            .eq(Student::getSchoolId, school.getSchoolId())
        ...
        // 统计合作企业的岗位
        List<InternshipPost> posts = internshipPostMapper.selectList(postWrapper);
        Map<String, Long> postTypeCountMap = new HashMap<>();
        for (InternshipPost post : posts) {
            String postName = post.getPostName();
            if (postName != null && !postName.trim().isEmpty()) {
        ...
        // 按数量降序排序，只取前10个
        AtomicInteger colorIndexAtomic = new AtomicInteger(0);
    ...
    public SchoolAdminDashboardDTO getSchoolAdminDashboard() {
        ...
        if (!colleges.isEmpty()) {
            ...
            for (Major major : majors) {
                ...
                List<Student> students = studentMapper.selectList(
                        new LambdaQueryWrapper<Student>()
                                .eq(Student::getMajorId, major.getMajorId())
                                .eq(Student::getInternshipStatus, 1) // 实习中
                                .select(Student::getStudentId, Student::getUserId)
                );
                // 通过关联user_info表过滤已删除的学生
        ...
        long maleCount = 0;
        long femaleCount = 0;
        List<Long> userIds = students.stream().map(Student::getUserId).filter(Objects::nonNull).collect(Collectors.toList());
        if (!userIds.isEmpty()) {
