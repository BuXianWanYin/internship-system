...
##### 2.13.4.2 工具类使用规范
- **EntityValidationUtil**：用于实体存在性和删除标志检查
  - `validateEntityExists(T entity, String entityName)`：检查实体是否存在且未删除
  - `validateIdNotNull(Long id, String idName)`：检查ID是否为空
- **使用原则**：
  - 优先使用工具类方法，避免重复代码
  - 工具类方法应保持简单，只处理通用逻辑
  - 特殊业务逻辑应在Service层处理
...
- [ ] 是否使用了工具类方法替代重复的验证逻辑（如EntityValidationUtil）
